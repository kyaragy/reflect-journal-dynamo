...
type Props = {
  ...
  saving: boolean;
  onClose: () => void;
  onSave: (reflection: WeeklyReflectionResult) => Promise<void>;
};
...
export default function WeeklyReflectionImportModal({ weekStart, sourceDays, saving, onClose, onSave }: Props) {
  ...
  const parsed = useMemo(() => {
    if (!rawInput.trim()) {
      return { reflection: null as WeeklyReflectionResult | null, error: null as string | null };
    }

    try {
      return {
        reflection: parseWeeklyReflectionImport(rawInput, weekStart, sourceDays),
        error: null,
      };
    } catch (error) {
      return {
    ...
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-stone-950/50 px-4 py-6">
      <div className="max-h-[95vh] w-full max-w-6xl overflow-auto rounded-3xl border border-stone-200 bg-stone-50 p-6 shadow-xl">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-xl font-medium text-stone-900">週次JSONを取り込む</h3>
            <p className="mt-1 text-sm text-stone-500">ChatGPTの返却JSONを貼り付けて、保存前に週次プレビューを確認します。</p>
          </div>
          <button
            type="button"
            onClick={onClose}
