...
import type { MonthlyReflectionResult, ThinkingDayRecord, ThinkingWeekRecord } from '../../domain/thinkingReflection';
...
export default function MonthlyReflectionPreview({ reflection, sourceWeeks, sourceDays }: Props) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-sky-200 bg-linear-to-br from-sky-50 via-white to-cyan-50 p-6 shadow-sm">
        ...
        <p className="mt-4 whitespace-pre-wrap text-base leading-8 text-stone-700">{reflection.monthly_summary}</p>
      </section>
      ...
      <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
        ...
        <ul className="mt-4 space-y-2.5">
          {reflection.looping_patterns.map((item) => (
            <li key={item} className="rounded-2xl bg-sky-50 px-4 py-3 text-base leading-8 text-sky-950">
              {item}
            </li>
          ))}
        </ul>
      ...
      <section className="rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 space-y-3">
          {reflection.source_weeks.map((sourceWeek) => {
            ...
            const weekDates = Array.from({ length: 7 }, (_, index) => format(addDays(parseISO(sourceWeek.week_start), index), 'yyyy-MM-dd'));
            const weekTags = Array.from(
              new Set(
                weekDates.flatMap((date) => sourceDayMap.get(date)?.entries.flatMap((entry) => entry.tags ?? []) ?? [])
              )
            );
                ...
                {weekly ? (
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    ...
                    <div className="rounded-2xl bg-white p-4 md:col-span-2">
                      ...
                      <p className="mt-2 whitespace-pre-wrap text-base leading-8 text-stone-700">{weekly.weekly_summary}</p>
                    </div>
